...
def _i140(a: A) -> dict[str, Any]:
    m: dict[str, Any] = {
        ...
        # Part 2 — petition type: 1.i NIW (advanced degree / exceptional ability)
    ...
    if len(family) > 1:
        p = family[1]
        m.update({
            "Line2a_Person2FamilyName[0]": p.get("family_name"),
            "Line2b_Person2GivenName[0]": p.get("given_name"),
            "Line2c_Person2MiddleName[0]": p.get("middle_name"),
            "Line2d_DateOfBirth[0]": p.get("dob"),
            "Line2e_CountryOfBirth[0]": p.get("country_of_birth"),
            "Line2f_Relationship[0]": p.get("relationship"),
        })
    return m
...
def _appendix_a(a: A) -> dict[str, Any]:
    ...
    for i, d in enumerate(degrees[:5]):
        ...
        box = _DEGREE_BOXES.get(str(d.get("level", "")).lower())
        if box:
            # blocks 4-5 use slightly different checkbox names on the PDF
            if i >= 3 and box == "Associate":
                box = "Associates"
            if i >= 3 and box == "Masters":
                box = "Master"
            m[f"{box}{sfx}"] = True
